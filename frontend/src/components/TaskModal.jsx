...
const API_BASE = 'https://taskflow-vbfj.onrender.com/api/tasks' // Ensure API_BASE is defined
...
const TaskModal = ({isOpen , onClose , taskToEdit , onSave , onLogout}) => {
    ...
    const handleSubmit = useCallback(async (e)=> {
        ...
        try{
            ...
            console.error(err)
            setError(err.message || "unexpected error occures")
  ...
  return (
    // Outer div (backdrop) handles clicks to close the modal
    <div
      className='fixed inset-0 backdrop-blur-sm bg-black/20 z-50 flex items-center justify-center p-4'
      onClick={onClose} // This will close the modal if clicked outside
    >
        {/* Inner div (modal content) prevents clicks from bubbling up */}
        <div
          className='bg-white border border-purple-100 rounded-xl max-w-md w-full shadow-lg relative p-6 animate-fadeIn'
          onClick={(e) => e.stopPropagation()} // This stops clicks inside the modal from closing it
        >
            <div className='flex justify-between items-center mb-6'>
                <h2 className='text-2xl font-bold text-gray-800 flex items-center gap-2'> {taskData.id ? 'Edit Task' : 'Create New Task'}
                </h2>
            ...
            <form action="" onSubmit={handleSubmit} className='space-y-4'>
                {error && <div className='text-sm text-red-600 bg-red-600 p-3 rounded-lg border border-red-100'>{error}</div>}
                <div>
                    <label htmlFor="" className='block text-sm font-medium text-gray-700 mb-1'>Task Title</label>
                ...
                <div>
                    <label htmlFor="" className='flex items-center gap-1 text-sm font-medium text-gray-700 mb-1'><AlignLeft className='w-4 h-4 text-purple-500'/>Description</label>
                    <textarea name="description" id="" rows='3' onChange={handleChange} value={taskData.description}className={baseControlClasses} placeholder='Add details of your task'></textarea>
                </div>
                <div className='grid grid-cols-2 gap-4'>
                    <div>
                        <label htmlFor="" className='flex items-center gap-1 text-sm font-medium text-gray-700 mb-1'>
                            <Flag className='w-4 h-4 text-purple-500'/> Priority
                        </label>
                        <select name="priority" id="" value={taskData.priority} onChange={handleChange} className={`${baseControlClasses} ${priorityStyles[taskData.priority]}`}>
                ...
                <div>
                    <label htmlFor="" className='flex items-center gap-1 text-sm font-medium text-gray-700 mb-2'>
                    ...
                    <div className='flex gap-4'>
                        {[{val : 'Yes' , label : "Completed"} , {val: 'No' , label: "In progress"}].map(({val,label}) => (
                            <label key={val} className='flex items-center'>
                                <input htmlFor="" type='radio' name='completed' value={val} checked = {taskData.completed === val} onChange={handleChange} className='h-4 w-4 text-purple-600 focus:ring-purple-500 border-gray-300 rounded'/>
