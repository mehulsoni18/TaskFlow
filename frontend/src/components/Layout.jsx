...
const Layout = ({ onLogout, user }) => {
  ...
  const fetchTasks = useCallback(async () => {
    ...
    try {
      ...
      const { data } = await axios.get('https://taskflow-vbfj.onrender.com/api/tasks/gp', {
  ...
  const stats = useMemo(() => {
    ...
    const completePercentage = totalCount ? Math.round((completedTasks / totalCount) * 100) : 0;
  ...
  const StatCard = ({ title, value, icon }) => (
    <div className='p-2 sm:p-3 rounded-xl bg-white shadow-sm border border-purple-100 hover:shadow-md transition-all duration-300 hover:border-purple-100 group'>
      <div className='flex items-center gap-2'>
        <div className='p-1.5 rounded-lg bg-gradient-to-br from-fuchsia-500/10 to-purple-500/10 group-hover:to-purple-500/20'>
          {icon}
        </div>
          ...
          <div className='xl:col-span-1 space-y-4 sm:space-y-6 '>
            <div className='bg-white rounded-xl p-4 sm:p-5 shadow-sm border border-purple-100'>
              <h3 className='text-base sm:text-lg font-semibold mb-3 sm:mb-4 text-gray-800 flex items-center gap-2'>
                <TrendingUp className="w-4 h-4 sm:w-5 sm:h-5 text-purple-500" />Task Statistics
              </h3>
              <div className='grid grid-cols-2 gap-3 sm:gap-4 mb-4 sm:mb-6'>
                ...
                <StatCard title='Completion Rate' value={`${stats.completePercentage} %`} icon={<Circle className='w-3.5 h-3.5 sm:w-4 sm:h-4 text-purple-500' />} />
              </div>
              <hr className='my-3 sm:my-4 border-purple-300' />
              <div className='space-y-2 sm:space-y-3'>
                <div className='flex items-center justify-between text-gray-700'>
                  <span className='text-xs sm:text-sm font-medium flex items-center gap-1.5'>
                    <Circle className='w-2.5 h-2.5 sm:w-3 sm:h-3 text-purple-500 fill-purple-500' /> Task Progress
                  </span>
                ...
                <div className='relative pt-1'>
                  <div className='flex gap-1.5 items-center'>
                    <div className='flex-1 h-2 sm:h-3 bg-purple-100 rounded-full overflow-hidden'>
                      <div className='h-full bg-gradient-to-r from-fuchsia-500 to-purple-600 transition-all duration-500' style={{ width: `${stats.completePercentage}%` }} />
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className='bg-white rounded-xl p-4 sm:p-5 shadow-sm border-purple-100'>
              <h3 className='text-base sm:text-lg font-semibold mb-3 sm:mb-4 text-gray-800 flex items-center gap-2'>
                <Clock className='w-4 h-4 sm:w-5 sm:h-5 text-purple-500' />Recent Activity
              </h3>
              <div className='space-y-2 sm:space-y-3'>
                {tasks.slice(0, 3).map((task) => (
                  <div key={task._id || task.id} className='flex items-center justify-between p-2 sm:p-3 hover:bg-purple-50/50 rounded-lg transition-colors duration-200 border border-transparent hover:border-purple-100'>
                    ...
                    <span className={`px-2 py-1 text-xs rounded-full shrink-0 ml-2 ${task.completed ? 'bg-green-100 text-green-700' : 'bg-fuchsia-100 text-fuchsia-700'}`}>{task.completed ? "Done" : 'Pending'}</span>
                  </div>
                ))}
