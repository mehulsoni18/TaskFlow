...
export async function registerUser(req,res) {
    ...
    if(password.length <8) {
        return res.status(400).json({success:false , message: "Password must be 8 characters atleast"});
    }
...
export async function updatePassword(req,res){
    ...
    try{
        ...
        if(!user){
            return res.status(400).json({success:false , message: 'User not foundt'});
        }
